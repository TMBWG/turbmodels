! gfortran -O2  hchf_v1p8.f90
! ifort    -O2  hchf_v1p8.f90
!*******************************************************************************
!
!    --- Grid generation code for a hemisphere-cylinder-hemisphere (HCH) ---
!
!
! This code generates a 3D grid over an HCH-type axisymmetric shape.
!
! Version 1.8 (July 26, 2017).
!
! v1.8: Added an option to generate a matching-HC grid: 'body_type=5'
!       The HC surface grid is generated in the same way as hcf_hc_v8p0.f90.
! v1.7: Added ellipsoid of revolution (prolate spheroid): 'body_type=4'
...
!
!       Note: This .k file is required in 'hch_coarsening_v1p2.f90' for
...
!       Note: These line files are required in 'hchf_coarsening_v1p2.f90' for
!             generating a series of coarse grids.
...
!        as defined below.
!
!   Note: If you want to regularly coarsen the grid afterwards (by 'hchf_coarsening_v1p2.f90'),
...
!----------------------------
! Axisymmetric body type:
!  1=Suboff model, 2=Hemisphere nose, 3=Sine func, 4=Ellipsoid, 5=Matching HC.
...
!----------------------------
! Parameters for the sine-shape function (body_type = 3).
...
!----------------------------
! Parameter for the ellipsoid of revolution (body_type = 4).

  real(dp) :: ellipse_rmax = 0.15

!----------------------------
! Only for body_type = 5: length of the sub-off tail.
! If negative, the length will be determined for smooth transition.

  real(dp) :: length_suboff_tail_hc5 = 2.5_dp
...
!*****************************************************************************
  ...
  !------------------------------
  ! Hemisphere nose (special case)
  !------------------------------

   if (body_type == 5) then
   ! Initial grid generation is performed over x=[0,1].
   ! In this option, we want L*(1-R_hemi) = body_length and L*R_hemi = 0.5,
   ! which gives L - 0.5 = body_length, and so, L = body_length + 0.5.

    write(*,*) "--- This is a matching HC case. body_length is the length of HC."

   ! Define L:
    write(*,*) " body_length    = ", body_length
    body_length = body_length + 0.5_dp

   ! This is R_hemi = 0.5/L.
    R_hemi = 0.5_dp/body_length
    write(*,*) " R_hemi input    = ", R_hemi
    write(*,*) " R_hemi adjusted for body_type=5 case: ", R_hemi
...
! (2)[Optional] Tecplot files for debugging.
  ...
  character(80) :: filename06 = "debug07_hemisphere_cylinder_surface0.dat"
...
! Strct grid
  ...
  real(dp)     :: xmax_hc5, x2m, scaling_suboff_tail_hc5
...
! For body_type == 5, the length of the suboff tail, (x3-x2), will be multiplied
! by the input value, length_suboff_tail_hc5, or the scaling will be determined
! for smooth transition if length_suboff_tail_hc5 < 0 .
...
!-------------------------------------------------------------------------------
! Matching HC (body_type == 5). To match the HC surface grid and the one
! generated by the HC grid generation code.
!-------------------------------------------------------------------------------

  matching_hc : if (body_type == 5) then
...
! - Full geometry
 ...
 full_geom_matching_hc : if (n_sections == 6) then
...
!    Uniform spacing
    ...
    if (i==nnodes_cylinder) x2m = node_body(nnodes)%x
...
!-------------------------------------------------------------------------------
! End of Matching HC or not.
!-------------------------------------------------------------------------------

 write(*,*) "     nnodes_circum = ", nnodes_circum
 write(*,*) "               x2  = ", x2
 write(*,*) "               x2m = ", x2m
 write(*,*) " Last spacing in x = ", x2-x2m
...
!*******************************************************************************
  ...
  !------------------------------------------------------------------
  ! Scaling factor for suboff tail.

   !------------------
   ! User specified
   if (length_suboff_tail_hc5 > 0.0_dp) then

    scaling_suboff_tail_hc5 = 2.0_dp*length_suboff_tail_hc5

   !------------------
   ! Smooth transition
   else

    scaling_suboff_tail_hc5 = (x2-x2m)/ ( (half*pi*Rd) / real(nr_gs,dp) )
...
! Map the front hemisphere information to create the rear hemisphere.
  ...
  !-------------------------------------------------------------------------------
  ! Special treatment for body_type = 5, to adjust the length of the suboff-tail.

   if (body_type == 5) then

        node_body(nnodes)%x = x2 + scaling_suboff_tail_hc5*(x1-node_body(i)%x)

        xmax_hc5 = max(xmax_hc5, node_body(nnodes)%x)

  !-------------------------------------------------------------------------------
  ! For others, nothing special. Just add a hemisphere of radius 0.5.
   else
  ...
  if (nquads_hem > 0) then
   ...
   do i = 1, nquads_hem_h
    ...
    quad_cyl(nquads_cyl,5) = node_hemi( quad_cyl(i,5) )
     ...
     write(*,*) " scaling_suboff_tail_hc5 = ", scaling_suboff_tail_hc5
     write(*,*) "  length_suboff_tail_hc5 = ", scaling_suboff_tail_hc5/2.0_dp
...
!******************************************************************************
 ...
 open(unit=4, file=filename06, status="unknown", iostat=os)
...
!*******************************************************************************
  ...
  if ( i == 1 .or. i == node_hemi(1) ) then
   ...
   ! To keep the HC shape. The HC part is in x = [0,1-R_hemi].
    if (body_type == 5 .and. xp < 1.0_dp-R_hemi) cycle
 ...
 ! Update the surface normals. Still x=[0,1] is assumed.
  ...
  if ( i == 1 .or. i == node_hemi(1) ) then
   ...
   ! To keep the HC shape. The HC part is in x = [0,1-R_hemi].
    if (body_type == 5 .and. node_body(i)%x < 1.0_dp-R_hemi) cycle
...
!*******************************************************************************
 ...
 real(dp)             :: r
   ...
   !(4)Ellipsoid of revolution
    ...
    elseif (body_type == 4) then

     r = shape_func_ellipse(xx)

   !------------------------------------------------------
   !(5)Special hemisphere forebody with suboff model rear body.

    elseif (body_type == 5) then
   ...
   !------------------------------------------------------
     ...
     write(*,*) " --- body_type must be an integer in [1,2,3,4]. Stop."
     stop
...
!********************************************************************************
! A special version of the shape function for a hemisphere-cylinder closed by
! the afterbody of the subboff model:
!
! Geometric Characteristics of DARPA Suboff Models,
! Nancy C. Groves, Thomas T. Huang, and Ming S. Chang,
! DTRC/SHD-1298-01, March 1989.
...
!
!********************************************************************************
 function shape_func_hc5(xx) result(r)

 real(dp), intent(in) :: xx
 real(dp)             :: r
 real(dp)             :: x, rmax, xi, rh, k0, k1, width, s

! Note: suboff-tail function is defined over x=[0,14.291666].
!       The tail is defined over x=[10.645833, 14.291666].

! The rear hemisphere grid is defined over x=[1-R_hemi, xmax_hc5].
...
!       since we need rmax=0.5 to connect to the HC.
   ...
   width = 14.291666_dp-10.645833_dp

   !s = [0,1] over x=[1-R_hemi, xmax_hc5]
    s = ( xx - (1.0_dp-R_hemi) ) / (xmax_hc5-(1.0_dp-R_hemi))

   !x = [10.645833, 14.291666]
    x = 10.645833_dp + width*s

    rmax = 0.5_dp

   if (xx <= 1.0_dp-R_hemi) then
    !Don't use this function in this domain.
    !But will be used for derivative at x=1-R_hemi.
    !So, be consistent there, i.e., constant r.
     r = rmax / body_length
     return
   endif
...
!********************************************************************************
! Ellipsoid of revolution (prolate spheroid)
!
!  Input: xx = [0,1] - Normalized x-coordinate.
! Output:  r         - Radial distance that determines the shape.
!
!********************************************************************************
 function shape_func_ellipse(xx) result(r)
 ...
 real(dp)             :: a
  ...
  a = 0.5_dp
  r = ellipse_rmax * sqrt( 1.0_dp - (xx-0.5_dp)**2/a**2 )

 end function shape_func_ellipse
