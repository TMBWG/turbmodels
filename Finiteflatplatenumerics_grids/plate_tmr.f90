...
program main
  ...
  real(dp), dimension(2*320+1) :: xspacing, xspacingff
  ...
  real(dp) :: aspect_ratio_edge, scale_factor

continue

  aspect_ratio_edge = 1._dp
  ...
  ! Stretch from xs_min to xs_ff for spacing off plate.

  xs_min  = 0._dp

  write(*,*) ' Stretch off plate...xs_ff=',xs_ff

  if ( x_stretching > 0 ) then

    dxs_min = aspect_ratio_edge*dys_min

    dy_at_xs_min = 1.0e+40_dp

    call stretch_function( ncell_x_le+1, xs_min, xs_ff, dxs_min, &
                           dy_at_xs_min, .false., xspacingff,    &
                           beta_x, n_unity_ar )

    xspacingff(ncell_x_le+1) = xs_ff

    dx1 = xspacingff(ncell_x_le+1)-xspacingff(ncell_x_le)
    dx2 = ( xs_ff - xs_min )/real( ncell_x_le, dp )

    write(*,"(1x,5(a,e12.5))") ' beta_x=',beta_x,&
    ' dx1=',dx1,' equal_spacing=',dx2,' ratio=',dx1/dx2

  else

    dxs_min = xs_ff/real(ncell_x_le,dp)

    do j=1,ncell_x_le+1
      xspacingff(j) = real(j-1,dp)*dxs_min
    enddo

  endif

  ! Stretch from xs_min to xs_max for spacing on plate.

  xs_min  = 0._dp
  xs_max  = 1._dp

  write(*,*) ' Stretch on plate...xs_max=',xs_max
 ...
 !y = y*scale_factor
  ...
  write(80,fmt1) pr,     'symmetry_z_strong',pr,1, 2, 1, ni, 1, nj   !k=constant
...
continue
  ...
  do outer = 1,20
    ...
    write(90,"(1x,a,i5,2e20.10)") ' outer,beta,target=',&
                                    outer,beta,y(n)-y_max

    if ( abs( y(n) - y_max ) < 1.0e-12_dp ) exit
