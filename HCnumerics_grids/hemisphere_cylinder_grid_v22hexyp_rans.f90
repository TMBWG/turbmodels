...
!
! - Generate a prismatic, tetrahedral, or mixed grid. (.ugrid .or b8.ugrid)
! - Generate an associated boundary condition file. (.mapbc)
! - Generate a line info file for line-implicit and/or line-agglom. (.lines_fmt)
! - Generate a file containing structured indices. (.k)
!
! This is Version 18hex (May 2015).
!
!
...
!
!  Input:
!              igrid_type = Element type. 1=Prism, 2=Tet, 3=Mixed
!                distance = Distance to the outer boundary
!                      x2 = Length of the hemisphere-cylinder
!             line_extent = 1(within a thin layer) or 2(to the outer boundary)
...
!
!   (2)Files used for CFD computations.
!
!     "hemisphere_'element_type'.mapbc"           !Boundary condition file (FUN3D)
!     "hemisphere_'element_type'.ugrid/.b8.ugrid" !Unstructured grid
!     "hemisphere.lines_fmt"                      !Lines for implicit solves
!
!   (3)File containing k1-k2-k3-k4-k5 structured indices.
!
!                          "hemisphere_tetprism.k"
!
! Katate Masatsuka, May 2015.
...
! Local variables
  ...
  real(dp) :: x1           !Left end of the cylidner.
  real(dp) :: x2           !Rear end coordiate of the cylidner
  ...
  real(dp) :: r_gs         !Raidus of the generating sector
...
! For smoothing applied on the disk triangulation
  ...
  logical :: generate_tec_files = .false.
...
!*******************************************************************************
! 0. Selecet the type of grid to generate.
!*******************************************************************************
...
!            \/             ----> y
!
! NOTE: The number of triangules = 1 + 3 + 5 + ... + 2*nr_gs-1 = nr_gs*nr_gs
!       The number of ndoes  = 1 + 2 + 3 + ... + nr_gs+1
!
...
!  Move newnode to the boudnary (rnode position)
...
!       the spacings of the elements below and above the nm-th node.
!
!*******************************************************************************
...
!  considering thet the cell expands to the outer boundary.
...
! Apply smoothing on the outer boundary
 ...
 smoothing_outer : do i_smoothing = 1, max_smoothing
...
!   Apply the stretching to the uniform spaceing:
...
!*******************************************************************************
  ...
  open(unit=16, file=filename_mapbc, status="unknown", iostat=os)
   ...
   write(16,*) 3       !Number of boundary parts (boundary conditions)
   write(16,*) 1, 4000 !Viscous wall in FUN3D
   write(16,*) 2, 7012 !Outflow in FUN3D
   write(16,*) 3, 5000 !Free stream in FUN3D
...
!*******************************************************************************
 ...
 write(*,*) "Grid generation successfuly completed."
...
!*******************************************************************************
  ...
  if (n_sections==6) then
   ...
   nquads_b = nquads_b + 2*nquads_cyl  ! + All over cylidner surface and outer
   allocate(quad(nquads_b,5))
  elseif (n_sections==1) then
   nquads_b = nnodes_circum * (nr-1) ! All over outflow boundary
   nquads_b = nquads_b + 2*nquads_cyl  ! + All over cylidner surface and outer
  ...
  allocate(prs(nprs,6) )
  nhex = nquads_cyl*(nr-1)          ! All over the cylidner part
  allocate(hex(nhex,8))
  ntet = 0
  allocate(tet(ntet,6) )
...
! Generate Hex
  ...
  do i = 1, nquads_cyl
   ...
   node5 = node_above(quad(nquads_b,1))
   node6 = node_above(quad(nquads_b,2))
   node7 = node_above(quad(nquads_b,3))
   node8 = node_above(quad(nquads_b,4))
...
!*******************************************************************************
 subroutine write_ugrid_file
  ...
  if ( b8_ugrid_format ) then
    ...
    !                    #hex
    write(9,'(7I10)') nnodes,   ntrias_b,    nquads_b,   ntet,    0, nprs, nhex
  endif
...
! Nodes
  do i = 1, nnodes
   write(9,'(3ES20.10)') node(i)%x, node(i)%y, node(i)%z
  end do

! Triangular faces = ntri
  if (ntrias_b > 0) then
   do i = 1, ntrias_b
    write(9,'(3I10)') tri(i,1), tri(i,2), tri(i,3)
   end do
  endif
...
! tet
  if (ntet > 0) then
   do i = 1, ntet
    write(9,'(4I10)') tet(i,1), tet(i,2), tet(i,3), tet(i,4)
   end do
  endif
...
! Prism
  if (nprs > 0) then
   do i = 1, nprs
    write(9,'(6I10)') prs(i,1), prs(i,2), prs(i,3), &
...
! Hex
  if (nhex > 0) then
   do i = 1, nhex
    write(9,'(8I10)') hex(i,1), hex(i,2), hex(i,3), &
...
!*                     shrinked to the requested dimension, n, if n < dim(x).
!*
!********************************************************************************
